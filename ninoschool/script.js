...
const distanceToFourth25 = currentLocation.distanceTo(fourthCheckpoint);
    ...
    nearestDistance.innerHTML = `Distance 2: ${Math.round(distanceToFirst)}m`;
...
if (distanceToSecond < 20 && !playedAudio100left) {
  ...
  const audio25left = document.getElementById('audioLeft'); // Assume this is the ID for the left turn audio
  audio25left.play();
...
document.addEventListener('DOMContentLoaded', function () {
  const resetButton = document.getElementById('resetNavigation');
  const audioElements = document.querySelectorAll('audio'); // Assuming your audio elements are tagged with <audio>

  resetButton.addEventListener('click', function() {
      // Reset all audio played flags
      playedAudio100right = false;
      playedAudio100left = false;
      playedAudio100left2 = false;
      playedAudio100right2 = false;
      playedAudio25right = false;
      playedAudio25left = false;
      playedAudio25left2 = false;
      playedAudio25right2 = false;
      ...
      // Optionally, recenter the map or reset the user's marker
      if (userLocationMarker) {
          map.removeLayer(userLocationMarker); // Removes the user location marker from the map
          userLocationMarker = null; // Clear the marker object
      }

      // Optionally, recenter the map to the starting point or an average point
      map.setView([avgLat, avgLng], 16.3); // Recenter map to the average coordinates

      console.log("Navigation and audios have been reset.");
  });
});
